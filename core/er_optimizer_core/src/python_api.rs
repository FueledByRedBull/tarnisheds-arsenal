...
impl From<OptimizeResult> for PyOptimizeResult {
    fn from(value: OptimizeResult) -> Self {
        Self {
            ...
            fai: value.stats.fai,
            arc: value.stats.arc,
            req_str: value.requirements[0],
            req_dex: value.requirements[1],
            req_int: value.requirements[2],
            req_fai: value.requirements[3],
            req_arc: value.requirements[4],
            scaling_str: value.effective_scaling[0],
            scaling_dex: value.effective_scaling[1],
            scaling_int: value.effective_scaling[2],
            scaling_fai: value.effective_scaling[3],
            scaling_arc: value.effective_scaling[4],
...
fn build_request(
    ...
    Ok(OptimizeRequest {
        ...
        min_combat_stats,
        locked_combat_stats,
        standard_max_upgrade: max_upgrade,
        somber_max_upgrade: max_upgrade.min(10),
        exact_upgrade: fixed_upgrade.is_some(),
        two_handing,
        dlc_scaling,
        scadutree_level,
        weapon_name,
